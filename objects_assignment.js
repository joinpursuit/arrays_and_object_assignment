...
// console.log(p1 === p2)
// ```
// the console will log out false. because it's not the same exact object
...
let myArray = [];
let myArray2 = [];
// a. Write a code block that
//takes an object variable and stores the *keys* in an array.  Then log the array.
for (let key in spanishNumbers){
  myArray.push(key); // question 3 a
  myArray2.push(spanishNumbers[key]) // question 3b
}
console.log(myArray); //3a
console.log(myArray2); //3b




// b. Write a code block that takes an object variable
//and stores the *values* in an array.  Then log the array.
...
let numberOfProperties = Object.keys(spanishNumbers).length;
...
for(let value in films){
  console.log(films[value].director);
...
let arrFirstNames = [];
let arrFullNames = [];
for (let firstNames in people){
  arrFirstNames.push(people[firstNames].firstName);
  arrFullNames.push(people[firstNames].firstName + " " + people[firstNames].lastName) ;
}
console.log(arrFirstNames);//6a
console.log(arrFullNames);//6b
// b. Create an array of strings called `fullNames` that
// contains the values for `“firstName”` and `“lastName”` from the object separated by a space.
...
//
// b) Create an array called `stolenCents`, iterate over deposits for each person and steal their cents! ... like Office Space or Superman 3.
//Calculate the decimal part of each value, add it to the `stolenCents` array and round down the value in the original object.
//
